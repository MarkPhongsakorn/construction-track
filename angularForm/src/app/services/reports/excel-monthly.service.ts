...
export class ExcelMonthlyService {
  constructor() { }
  ...
  exportToExcel(
    ...
    for (let i = 0; i < sheetName.length; i++) {
      ...
      cellD6.value = 'โครงการ: ' + project_name[i];
      ...
      cellA8.value = 'ช่วงเวลา: ' + period_name1[i] + ' สถานะ: ' + sta_name1[i] + ' ระดับฝน: ' + rain_level1[i] + ' เวลาฝนตก: ' + rain_start1[i] + ' น. ถึง ' + rain_end1[i] + ' น.';
      ...
      cellE8.value = 'ช่วงเวลา: ' + period_name2[i] + ' สถานะ: ' + sta_name2[i] + ' ระดับฝน: ' + rain_level2[i] + ' เวลาฝนตก: ' + rain_start2[i] + ' น. ถึง ' + rain_end2[i] + ' น.';
      ...
      cellA9.value = 'เวลาลงตรวจ ' + inspect_start[i] + ' - ' + inspect_end[i] + ' น.';
      ...
      cellE9.value = 'เวลาทำงาน ' + work_start[i] + ' - ' + work_end[i] + ' น.';
      ...
      for (let j = 0; j < 19; j++) {
        ...
        cellLaborName.value = labor_name[i][j] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
      ...
      for (let j = 0; j < 19; j++) {
        ...
        cellLaborNum.value = labor_num[i][Number(j)] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
      ...
      for (let j = 0; j < 17; j++) {
        ...
        cellWorkNum.value = work_num[i][Number(j)] || '';
      ...
      for (let j = 0; j < 17; j++) {
        ...
        cellWorkDetail.value = work_detail[i][j] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
      ...
      for (let j = 0; j < 18; j++) {
        if (j < 9) {
          ...
          cellToolName.value = tool_name[i][j] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
      ...
      for (let j = 0; j < 18; j++) {
        ...
        if (j < 9) {
          ...
          cellToolNum.value = tool_num[i][Number(j)] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
      ...
      for (let j = 0; j < 9; j++) {
        ...
        const matName = mat_name[i][j] || '';
        const matUnit = mat_unit[i][j] || '';
        let nameAndUnit = '';
        if (matName !== '' && matUnit !== '') {
      ...
      for (let j = 0; j < 9; j++) {
        ...
        cellMatNum.value = mat_num[i][Number(j)] || ''; // ใช้ข้อมูลถ้ามี หรือว่างเป็นสตริงถ้าไม่มีข้อมูล
